...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		foreach($query as $article) {
			...
			$article['pic'] = $article['pic']?pic_get($article['pic'], '', $article['thumb'], $article['remote']):'';
		...
		$hottag=C::t('portal_hottag')->gettag($keyword);	
		$data=array(
			'name'=>$keyword,
			'searchnum'=>1,
			'sort'=>999,
		...
		if($hottag) {
			$data['searchnum']=$hottag['searchnum']+1;
			C::t('portal_hottag')->update($hottag['id'],$data);
		} else {
			$data['isactived']=0;
			C::t('portal_hottag')->insert($data);
		}
