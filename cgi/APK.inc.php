...
class APK
{
	...
	function __construct($c){
		$this->C = $c;
		if($this->C->apk['log'])$this->fp = fopen($this->C->apk['log'],'a+');
	}
		...
		if ($res === TRUE) {
			...
			if ($xml){
				try {
					...
					$this->log("parse xml error");
					return "parse xml error";
				}
			}else{
				$this->log("get xml file failed");
				return $xml_file." not fount";
			}
		}else{
			$this->log("open ".$apk_file." failed:".$res);
			return "open apk file error:".$res;
		}
		return false;
	}
